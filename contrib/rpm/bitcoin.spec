...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9316
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9317
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19316
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19317
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 9316
	%{_sbindir}/semanage port -d -p tcp 9317
	%{_sbindir}/semanage port -d -p tcp 19316
	%{_sbindir}/semanage port -d -p tcp 19317
	for selinuxvariant in %{selinux_variants}; do
