USCoin Core latest version is available from:

  <https://uscoin.tech/download/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/uscoin-project/uscoin/issues>
...
updates will be released anymore. Without security updates, using a uscoin
...
ECDSA signatures inside USCoin transactions now use validation using
...
Previous versions of USCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of USCoin Core was to raise the default minimum
relay fee.

USCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
USCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running uscoind without having to do any manual configuration.
...
USCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. USCoin Core will relay transactions with
...
entirely in USCoin Core 0.13, and it is at this time undecided whether the
...
USCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
USCoin Core automatically creates a hidden service to listen on, without
manual configuration. USCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if USCoin Core is listening, and
...
USCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals USCoin
Core to use floating fees. By default, USCoin Core will use floating
fees.
...
(default: `0.0002` USDC/kB).

At all times, USCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) USDC.
Furthermore, USCoin Core will never create transactions paying less than
...
`uscoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 29316 to a RPC server bound on localhost on port 19316 do:

    stunnel -d 29316 -r 127.0.0.1:19316 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /uscoinrpc>
        ProxyPass http://127.0.0.1:9316/
        ProxyPassReverse http://127.0.0.1:9316/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass uscoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< uscoinrpc:<password>
    </Location>
...
Restarting uscoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in uscoin.conf
...
For example:
    ...
    $ src/uscoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The USCoin Core code base has
...
- `uscoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `uscoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the USCoin scripting system that in combination with
...
USCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of USCoin Core.
...
The two phases are distinct in the USCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming USCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for USCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each USCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of uscoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of uscoins being spent is exactly
...
- **Increased security for multisig:** USCoin addresses (both P2PKH addresses
  ...
  believed to be provided by USCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of USCoin's current ledger (called the UTXO set),
  ...
  note that USCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  USCoin users to individually opt-in to almost any change in the USCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and USCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the USCoin
client to USCoin Core 0.13.2 to signal for segwit support. When a
...
Since USCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `uscoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `uscoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
